...
public class MoveToPoints : MonoBehaviour 
{
	...
	void Awake()
	{
		...
		{
			if (GetComponent<CharacterMotor>() == true)
				characterMotor = GetComponent<CharacterMotor>();
			else
            {
				if (GetComponentInChildren<CharacterMotor>() == true)
					characterMotor = GetComponentInChildren<CharacterMotor>();
				else
					Debug.LogError("'CharacterMotor' script is missing.");
			}
			...
			{
				if (GetComponentInChildren<EnemyAI>() == true)
		...
		{
			if (GetComponentInChildren<Rigidbody>() == true)
				rigid = GetComponentInChildren<Rigidbody>();
			else
				Debug.LogError("'Rigidbody' script is missing.");
		}
	...
	void FixedUpdate()
	{
		if(transform.tag != "Enemy")
		{
			if(!arrived && waypoints.Count > 0)
			{
				...
				rigid.MovePosition(transform.position + (direction.normalized * speed * Time.fixedDeltaTime * gameManager.timeManager.GetGlobalTimeScale()));
			}
		}
	}
