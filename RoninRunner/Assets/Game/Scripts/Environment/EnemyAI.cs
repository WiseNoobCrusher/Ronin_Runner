...
public class EnemyAI : MonoBehaviour 
{
	...
	void Awake()
	{
		...
		if (GetComponentInChildren<Animator>() == true)
			animatorController = GetComponentInChildren<Animator>();
		else
			Debug.LogError("'Animator' script is missing.");
	...
	void Update()
	{
		...
		if (gameManager.timeManager.GetGlobalTimeScale() != 1.0f)
		{
			rigid.drag = 2 * gameManager.slowMultiplier;
			animatorController.speed = 1 / gameManager.slowMultiplier;
		}
		else
		{
			rigid.drag = tempDrag;
		...
		{	
			//notify animator
			if(animatorController && !moveToPointsScript.enabled)
