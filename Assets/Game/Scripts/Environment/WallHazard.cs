...
public class WallHazard : MonoBehaviour
{
    ...
    {
        // Moves the wall by the amount given, if the x value isn't farther than the target
        if (newX <= target.transform.localPosition.x)
        {
            ...
            Debug.LogError("The value of X provided is farther than the target, please enter a value that is less than the x value of the intended target");
    }
